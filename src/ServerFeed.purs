...
updateJoins updt rooms =
  ...
  in
    foldl (\m (Tuple k ru) -> Map.alter (combineRoom ru) k m) (fromMaybe (Map.empty) rooms) ((Map.toUnfoldable updt) :: Array (Tuple RoomId RoomUpdate))
...
serverState :: forall m. MonadFRP m => SessionInfo -> m (WeakDynamic (Map RoomId RoomData))
serverState si = do
